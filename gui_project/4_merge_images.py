...
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", \
        filetypes=(("PNG 파일", "*.png"), ("모든 파일", "*.*")),\
            initialdir=r"C:\Users\JSY\Desktop\PYTHONWORKSPACE") 
...
def merge_image():
    ...
    widths, height = zip(*(x.size for x in images))
    
    max_wdth, max_height = max(widths), sum(height)
    
    # 스케치북 준비
    result_img = Image.new(("RGB"), (max_wdth, max_height), (255,255,255)) # 배경 흰색
    y_offset = 0 # y 위치
    # for img in images:
...
# 3. 파일 포맷 옵션
# 파일 포맷 옵션 레이블
ldl_format = Label(frame_option, text="간격", width=8)
